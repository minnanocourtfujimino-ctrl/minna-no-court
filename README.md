...
枠ごとの「参加リスト」は、チャット機能を持たない人数集計だけの仕組みです。収集するのは学年とニックネーム(本名不要)のみで、氏名・連絡先などの個人情報は収集しません。サイト上に表示されるのは人数だけです。参加者同士が連絡を取り合う場所は提供せず、集合場所は常に施設の入口、当日その場で解散する運用です。
